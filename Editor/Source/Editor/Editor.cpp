...
public:
	...
	CustomPanel& SetName(const char* n)
	{
		name = n;
		return *this;
	}
	CustomPanel& AddView(std::shared_ptr<View> view)
	{
...
static void SaveRenderSettings(const fs::path& projectPath)
{
	auto settings = *Renderer::Settings::Instance();


	// auto json = JsonObject()
	// 	.Property("resolution", JsonArray().Elements(*settings.resolution))
	// 	.Property("shadows", *settings.shadows)
	// 	.Property("deferred", *settings.deferred)
	// 	.Property("postProcess", JsonObject()
	// 		.Property("bloom", settings.postProcess->bloom)
	// 		.Property("exposure", settings.postProcess->exposure)
	// 		.Property("inlineExposure", settings.postProcess->inlineExposure))
	// 
	// 	.Property("bloom", JsonObject()
	// 		.Property("bloom", settings.bloom->)
}
}