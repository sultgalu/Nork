...
{
	ImGui_ImplGlfw_InitForVulkan(Vulkan::Window::Instance().glfwWindow, true);
	InitImguiForVulkan();
}
EditorPass::~EditorPass()
{
...
void EditorPass::OnFramebufferResized() {
	auto inFlightFb = fbUI; // wait for rendering to finish before deleting the FB
	Commands::Instance().OnRenderFinished([inFlightFb]() {});
	CreateFramebuffer();
}
void EditorPass::CreateFramebuffer()
{
...
void EditorPass::CreateRenderPassUI()
{
...
void EditorPass::InitImguiForVulkan()
{
	CreateRenderPassUI();
	CreateFramebuffer();
...
void EditorPass::OnTransferCommands() {
	Editor::Get().Update();
}
