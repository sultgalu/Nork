...
namespace Nork::Physics {
	...
	public:
		...
		static std::vector<glm::vec3> FaceOnFace(const Collider& faceShape, const Collider& faceShape2, const uint32_t faceIdx, const uint32_t faceIdx2, bool clipDepth = true);
		static std::vector<glm::vec3> PointsOnFace(const Collider& faceShape, const uint32_t faceIdx, const std::vector<glm::vec3>& points);
	};
}
