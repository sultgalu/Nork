...
uniform float sunRad2 = 0.200f;
uniform float atmThick = 0.05f;
uniform float density = 20.33f;
uniform float density2 = 20.33f;
uniform float sunBorderStr = 0.33f;
...
void main ()
{
    ...
    dist = clamp(dist, 0, sunRad) * (1.0f / sunRad);
    ...
    // --------------------------------
    float dist2 = distance(lightPos, worldPos);
